...
/* The draft, dressed to the house. Panels, gold micro-caps, one gold
   action, hairline links, tabular naira. Every line editable; nothing
   saved until the owner taps. The price starts from the ledger and is
   his to change; the model never set it. */
...
export default function ReviewDraft({
  ...
  const [lines, setLines] = useState<Line[]>(
    draft.lines.map((l) => ({
      ...
      label: l.pieceName || l.description || "Line",
      quantity: l.quantity,
      unit: l.unit,
      unknown: l.unknown,
  ...
  return (
    <div className="grid gap-5">
      <div>
        ...
        <p className="mt-2 font-serif text-[26px]">Check what the book read.</p>
        <p className="mt-2 text-[14px] leading-relaxed text-dusk">
          Nothing is saved until you create it. Fix a line, set a price, or drop what does not belong.
        </p>
      </div>
      ...
      <div className="flex items-end justify-between gap-4">
        ...
        {missingPrice && (
          <p className="max-w-[140px] text-right text-[12px] text-mist">One line still needs your price.</p>
        )}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-5">
        ...
        <button type="button" onClick={() => window.location.reload()} className="link-hair text-[12px] text-dusk">
