...
/* The house's line to a vision model, now single-purpose: given one
   ...
   instead of a default guess. A pool is a shell: eight corners, the rim
   opening and the floor. Any other surface is one plane: four corners.
   The corners are approximate (a VLM is not a surveyor), so the finder
   tightens them against the segmentation masks; here we only need the
   rough skeleton. */
export type ShellCorners = { shape: "shell"; rim: VisualizerPoint[]; floor: VisualizerPoint[] };
...
export function normalizeCorners(input: unknown, wantShell: boolean): SurfaceCorners {
  ...
  if (wantShell) {
    ...
    const floor = normalizeQuad(input.floor);
    if (!rim || !floor) throw new VisualizerAiError("no-shell-corners");
    return { shape: "shell", rim, floor };
  }
...
export async function findSurfaceCorners(args: CornerArgs): Promise<SurfaceCorners> {
  ...
  const inputSchema = wantShell
    ? {
        ...
        additionalProperties: false,
        properties: {
          rim: quadSchema("The four corners of the pool's TOP opening where the interior wall meets the deck."),
          floor: quadSchema("The four corners of the pool FLOOR (the bottom plane)."),
        },
        required: ["rim", "floor"],
      }
    : {
  ...
  const instruction = wantShell
    ? "Mark the eight interior corners of the empty pool basin: the four rim (top opening) corners and the four floor corners. tl is the far-left, tr the far-right, br the near-right, bl the near-left, in the image plane. Follow the perspective; corners may be approximate."
